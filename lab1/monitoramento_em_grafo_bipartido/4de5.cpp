...
int monitoramento_em_grafo_bipartido( ListGraph &g, NodeName &vname, ListGraph::NodeMap<double> &custo, ListGraph::NodeMap<int> &solucao)
{
  ...
  for(ListGraph::NodeIt v(g); v != INVALID; ++v) {
    sprintf(namme,"V_%s",vname[v]);
    x[v] = model.addVar(0.0, 1.0, custo[v],GRB_CONTINUOUS,namme); }
  model.update();
  try {
    // Declarar/adicionar as variaveis, coeficientes da funcao objetivo
    // dar o comando
    // model.update();	
    // e colocar as restricoes do problema
    // o comando para resolver o LP estah comentado logo abaixo.
