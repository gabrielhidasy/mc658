...
public:
  Steiner_Instance(Digraph &graph,DiNodeName &vvname,
		   DiNodePos &posx,DiNodePos &posy,
		   ArcWeight &eweight,
		   int nt,vector <DiNode> &V); // first nt nodes are terminals
  Digraph &g;
  DiNodeName &vname;
  DiNodePos &px;
  DiNodePos &py;
  ArcWeight &weight;
  int nt,nnodes;
  vector <DiNode> &V; // Node V[0] is the root. Nodes V[1], ... , V[nt-1] are the destination
};

Steiner_Instance::Steiner_Instance(Digraph &graph,DiNodeName &vvname,
				   DiNodePos &posx,DiNodePos &posy,
				   ArcWeight &eweight,
				   int nterm, vector <DiNode> &V):
...
protected:
  void callback()
  {
    ...
    try {
      typedef Digraph::ArcMap<double> CapacityType;
      Digraph &g = T.g;
      CapacityType capacity(g);
      DiCutMap cut(g);
      double vcut;
	...
	if (vcut < 1.0-EPS) {// found violated cut
	  for (Digraph::ArcIt a(g); a!=INVALID; ++a) 
...
int ReadListDigraphSteiner(string steiner_filename,
			   ListDigraph &g,DiNodeName &vname,
			   ArcWeight &weight,
			   ListDigraph::NodeMap<double>  &px,
			   ListDigraph::NodeMap<double>  &py,
