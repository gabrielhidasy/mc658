...
bool ReadCFLPInstance(string filename, ListDigraph &g,
                      ...
                      ListDigraph::ArcMap<double>& edge_weight,
                      ListDigraph::NodeMap<double>& facility_weight,
                      ListDigraph::NodeMap<int>& facility_capacity,
...
#if __cplusplus >= 201103L
  std::unordered_map<string,DiNode> nodemap;
#else
  std::tr1::unordered_map<string,DiNode> nodemap;
#endif
...
int main(int argc, char *argv[])
{
  ...
  Digraph::ArcMap<int> ecolor(g, NOCOLOR); // color of edges
  Digraph::ArcMap<double> edge_weight(g);
  Digraph::NodeMap<double> facility_weight(g);
  Digraph::NodeMap<int> facility_capacity(g);
  ...
  if (argc==2) { ReadCFLPInstance(string(argv[1]),g,vname,edge_weight,
    ...
    nF = atoi(argv[2]);
    Cap = atoi(argv[3]);
    vector <DiNode> Client(1000);
    vector <DiNode> Facility(1000);
    for (int i=0;i<nC;i++) {
      Client[i] = g.addNode();
  ...
  for(Digraph::NodeIt v(g); v != INVALID; ++v) {
    if (vtype[v] == FACILITY) {
      ...
      for (Digraph::InArcIt e(g, v); e != INVALID; ++e) {
        num_customers += x[e];
      }
      model.addConstr(num_customers <= facility_capacity[v]);
    } else if(vtype[v] == CLIENT) {
      GRBLinExpr num_facilities = 0;
      for(Digraph::OutArcIt e(g, v); e != INVALID; ++e) {
          num_facilities += x[e];
          DiNode fac = g.runningNode(e);
  ...
  try {
    ...
    for(Digraph::NodeIt v(g); v != INVALID; ++v) {
      ...
      switch(vtype[v]) {
      case FACILITY:
        val = y[v].get(GRB_DoubleAttr_X);
        if (fabsl(val - 1.0) < EPS) {
          vcolor[v] = RED;
