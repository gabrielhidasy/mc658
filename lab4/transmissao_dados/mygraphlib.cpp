			     ...
			     ListGraph::NodeMap<double>& posy)
{
  ...
  //for (LstGraph::NodeIt vv(g); vv!=INVALID; ++vv) {
  // printf(" %d   ( %lf , %lf )\n",g.id(vv),posx[vv],posy[vv]);
  //}
  //printf("-------------------------------------------------------\n");
  return(true);
		    ...
		    vector<double>& q)
{
  ...
  if (NNodes<=0)
    {
      cout << "Wrong number of vertices in file " << filename<< ".\n";
      exit(0);
    }
  if (NEdges <=0)
    {
      cout << "Wrong number of edges in file " << filename<< ".\n";
      exit(0);
    }
